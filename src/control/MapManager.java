    ...
    public void checkCollisions(GameEngine engine) {
        ...
        if (mario.getY() > (48 * 16)) {
            if (map.getLives() == 0) engine.setGameStatus(GameStatus.GAME_OVER);
                ...
                int lives = map.getLives(); // Save the current lives before the game resets
                GameEngine.playSound("death");
                engine.createMap("map-01", isMultiplayer);
                map.setLives(lives - 1); // Decrement the remaining lives from the previous save
            }
        }
